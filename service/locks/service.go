...
type LockService interface {
	...
	NewLock(name, ownerID string, lockTTL time.Duration) (*Lock, error)
}
...
type lockService interface {
	...
	Claim(name, ownerID string, lockTTL time.Duration) error
	...
	Update(name, ownerID string, lockTTL time.Duration) error
